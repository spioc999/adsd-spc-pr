...
def handle_active_connection_lost(connection_id, current_node_id):
    ...
    If client remove it from ActiveConnections and from topics.
    If broker remove it from active connections and communicate to supervisor connection lost.
...
def handle_command_port(port_value, connection_id, current_node_ip, current_node_port):
    ...
    If received the port command then the connection become a broker connection.
    """
