...
def send(id_, conn, comando):
    ...
    uuid_message = comando["parametri"]["uuid"]
    timestamp = comando["parametri"]["timestamp"]

    risposta = dict()
    risposta["uuid"] = uuid_message
    risposta["topic"] = topic
    risposta["message"] = message
    risposta["timestamp"] = timestamp
...
def connection_manager_thread(id_, conn):
    stato = "CONNESSO"               # CONNESSO, IN-USCITA
